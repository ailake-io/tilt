...
DECLARACOES DE TOPO
  ...
  fonte X:              tipo: csv|json|postgres|duckdb|mysql|clickhouse|elasticsearch|opensearch|kafka|s3|spark ; caminho:/url: (env "VAR")
...
BUILTINS
  ...
  mongo_agregar
  ler_s3 escrever_s3 listar_s3 apagar_s3 copiar_s3 cabecalho_s3
  ...
  executar_sql "<url>" "<sql>"   (postgres://, sqlite://, duckdb://, mysql:// ou clickhouse://; INSERT/DDL)
  spark_sql "<url-livy>" "<sql>" / spark_executar "<url-livy>" "<codigo>"  ([lingua: "scala"|"pyspark", conf: {...}]; sessao Livy reusada, nao fechada)
  es_buscar "<url>" "<dsl>" / es_executar "<url>" "<metodo>" "<caminho>" [corpo]  (elasticsearch://; busca DSL e REST generico)
