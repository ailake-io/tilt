...
| Semântica | tipos, segredos, dispositivos, referências de ferramenta, resolução de nomes em `passos:`/`executar:` (`T030`), shape solver de `densa`/`linear` + formas literais `conv2d`/`norma_lote`/`matmul`/`reformar`/`transposta` (`T012`), contratos tensor em funções locais com `_` e atribuições diretas a membros de mapa, inferência de tipos conservadora (`T011`: operadores, builtins, métodos, retorno) | shape solver de funções genéricas sem contrato e de fluxos condicionais com tipos desconhecidos, inferência de tipos além do subconjunto conservador |
